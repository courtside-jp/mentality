...
async function loadPlayersFromAPI() {
  ...
  try {
    ...
    const url = `https://courtside-jp.github.io/mentality/data.json?v=${Date.now()}`;
    ...
    window._cachedPlayers = allPlayers.map(p => ({
      ...
      stl:        p.stl != null ? parseFloat(p.stl) : null,
      blk:        p.blk != null ? parseFloat(p.blk) : null,
      fg:         p.fg  != null ? parseFloat(p.fg)  : null,
      fg3:        p.fg3 != null ? parseFloat(p.fg3) : null,
      gp:         p.gp  != null ? p.gp  : null,
      min:        p.min != null ? parseFloat(p.min) : null,
      height:     p.height || "",
      dob:        p.dob || "",
      pos:        p.pos || "",
