// ===== \u30d0\u30c3\u30b7\u30e5\u30ec\u30d3\u30e5\u30fc\u30c6\u30f3\u30d7\u30ec\u30fc\u30c8 =====
window.SNK_REVIEW_TEMPLATE = `■ 1. 基本情報
正式なモデル名：
日本での発売日：　／　アメリカでの発売日：
日本での定価（税込）：
販売されたカラー一覧：
主な着用選手：
選手が実際に試合で着用したことを確認できる情報（試合日・出典）：
...
■ 3. 機能性評価（100点満点）
※点数は推測だけで決めず、複数のレビューサイトを比較した上で数値化すること
...
■ 6. ジム・トレーニング適性
※「バスケシューズだからジムにも向いている」と単純に判断せず、ソールの高さ・安定性・クッション・重量・足首の可動域などから評価すること
...
■ 7. 買うべき人
（例：グリップを最優先する人／コート感覚を重視する人／ディフェンスや切り返しが多い人／屋外でも使用したい人／復刻モデルや中古品を探している人）

■ 8. 見送るべき人
（例：クッション性を最優先する人／膝や足裏に不安がある人／体重が重い人／長時間プレーする人／センターやインサイド中心の選手／長距離ランニングにも使いたい人）
...
■ 11. ソース元
※海外の信頼できるバッシュレビューサイトを優先し、各情報についてどのソースを根拠にしたか分かるようにURLも明記すること
...
const SNK_PERF_INFO = {
  grip: `床をどれだけ「噛む」かの指標<br><span style="color:#2a9d5c;">高い→</span>急停止・切り返しでも滑りにくく安心<br><span style="color:#C9082A;">低い→</span>踏ん張りが効かず、方向転換で滑りやすい`,
  cushion: `着地の衝撃をどれだけ吸収するかの指標<br><span style="color:#2a9d5c;">高い→</span>着地が柔らかく足への負担が少ない<br><span style="color:#C9082A;">低い→</span>着地が硬く感じられ疲労がたまりやすい`,
  fit: `足と靴の一体感・隙間の少なさの指標<br><span style="color:#2a9d5c;">高い→</span>足がしっかりホールドされ操作しやすい<br><span style="color:#C9082A;">低い→</span>靴の中で足がズレやすく違和感が出やすい`,
  support: `足全体を包み込みブレを抑える力の指標<br><span style="color:#2a9d5c;">高い→</span>激しい動きでも足がしっかり守られる<br><span style="color:#C9082A;">低い→</span>素早い動きの中で不安定さを感じやすい`,
  stability: `着地・切り返し時に足元がグラつかないかの指標<br><span style="color:#2a9d5c;">高い→</span>着地が安定し捻挫などのリスクが下がる<br><span style="color:#C9082A;">低い→</span>不安定でぐらつきやすい`,
  bounce: `沈んだクッションが弾き返す力（反発力）の指標<br><span style="color:#2a9d5c;">高い→</span>ジャンプや連続動作で弾む感覚を得やすい<br><span style="color:#C9082A;">低い→</span>反発が弱く沈んだままの感覚になりやすい`,
  breathability: `靴の中の熱や湿気をどれだけ逃がせるかの指標<br><span style="color:#2a9d5c;">高い→</span>蒸れにくく快適<br><span style="color:#C9082A;">低い→</span>汗がこもりやすく蒸れやすい`,
  weight: `靴自体の軽さの指標<br><span style="color:#2a9d5c;">高い→</span>足運びが軽くスタミナを温存しやすい<br><span style="color:#C9082A;">低い→</span>重さを感じやすく疲れやすい`,
  sizeFeel: `表記サイズ通りに履けるかの指標<br><span style="color:#2a9d5c;">高い→</span>安心して通常サイズを選びやすい<br><span style="color:#C9082A;">低い→</span>サイズ選びに注意が必要（ハーフアップ／ダウンなど）`,
  widthFit: `さまざまな足幅への対応力の指標<br><span style="color:#2a9d5c;">高い→</span>幅広・幅狭どちらの足にも合わせやすい<br><span style="color:#C9082A;">低い→</span>特定の足幅だと窮屈・余りを感じやすい`,
  durability: `素材やソールがどれだけ長持ちするかの指標<br><span style="color:#2a9d5c;">高い→</span>激しい使用にも耐えやすい<br><span style="color:#C9082A;">低い→</span>消耗が早く傷みやすい`,
  outdoor: `屋外コートでの使用にどれだけ耐えられるかの指標<br><span style="color:#2a9d5c;">高い→</span>アスファルトなど荒い路面でも長く使える<br><span style="color:#C9082A;">低い→</span>ソールの消耗が早い`
};
...
async function openSnkModal(id) {
  ...
  const buildImgSectionHtml = (title, arr, credit, scrollId) => {
    ...
    <div id="${scrollId}" onscroll="snkUpdateGalleryDots('${scrollId}','${dotsId}')" style="display:flex;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;gap:0;border-radius:10px;margin:0 auto 8px;max-width:520px;">
      ...
      <div style="position:relative;flex:0 0 100%;width:100%;height:280px;scroll-snap-align:center;border-radius:10px;overflow:hidden;background:var(--bg3);">
        <img src="${img}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:blur(30px) brightness(0.8) saturate(1.3);transform:scale(1.3);">
  ...
  const otherWearersArr = (s.otherWearers || '').split(/[／,\n]/).map(x => x.trim()).filter(Boolean);
  const otherWearersHtml = otherWearersArr.length ? `
    <div style="font-size:11px;color:var(--tx3);margin:-2px 0 12px;line-height:1.7;">
      他にもこのモデルを着用した選手: ${otherWearersArr.join('、')}
    </div>` : '';
  const wornGalleryHtml = `
    ${(!wornImgs.length && otherWearersArr.length) ? '<div style="font-size:11px;font-weight:700;color:var(--tx3);margin-bottom:8px;">実際の着用</div>' : ''}
    ${buildImgSectionHtml('実際の着用', wornImgs, s.wornImageCredit, 'snkGalleryScroll_worn')}
    ${otherWearersHtml}`;
  const hasWornSection = wornImgs.length || otherWearersArr.length;
  const imgGallery = (imgs.length || hasWornSection) ? `
    ${productGalleryHtml}
    ${(imgs.length && hasWornSection) ? '<div style="height:1px;background:rgba(0,0,0,0.1);margin:4px 0 16px;"></div>' : ''}
    ${wornGalleryHtml}
  ` : '';
  ...
  body.innerHTML = `
    ...
    <div style="position:relative;overflow:hidden;display:flex;align-items:center;gap:16px;margin-bottom:14px;padding:16px 18px;background:linear-gradient(135deg, ${scoreColor}20, var(--bg3) 65%);border:1px solid ${scoreColor}40;border-radius:16px;">
      <div style="position:relative;width:78px;height:78px;flex-shrink:0;border-radius:50%;background:conic-gradient(${scoreColor} ${score * 3.6}deg, var(--bd) 0deg);display:flex;align-items:center;justify-content:center;box-shadow:0 0 18px ${scoreColor}50;">
        <div style="width:66px;height:66px;border-radius:50%;background:var(--bg);display:flex;flex-direction:column;align-items:center;justify-content:center;">
          <div style="font-size:26px;font-weight:800;color:${scoreColor};letter-spacing:-1px;line-height:1;">${score}</div>
          <div style="font-size:8px;color:var(--tx3);letter-spacing:1px;margin-top:2px;">/ 100</div>
        </div>
      </div>
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:6px;">
          <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;color:var(--tx3);">OVERALL RATING</div>
          <div style="font-size:20px;font-weight:800;color:${scoreColor};line-height:1;">${snkScoreGrade(score)}</div>
        </div>
        <div style="height:7px;background:var(--bd);border-radius:4px;overflow:hidden;">
          <div style="height:100%;width:${score}%;background:linear-gradient(90deg, ${scoreColor}, ${scoreColor}cc);border-radius:4px;"></div>
        </div>
      </div>
    </div>
...
async function submitSneaker() {
  ...
  const otherWearers = document.getElementById('sneakerOtherWearers') ? document.getElementById('sneakerOtherWearers').value.trim() : '';
...
function openNewSneaker() {
  ...
  if (document.getElementById('sneakerOtherWearers')) document.getElementById('sneakerOtherWearers').value = '';
...
async function editSneaker(id) {
  ...
  if (document.getElementById('sneakerOtherWearers')) document.getElementById('sneakerOtherWearers').value = d.otherWearers || '';
