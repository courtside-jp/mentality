...
async function loadHomeFeed() {
  ...
  try {
    ...
    _homeFeedCache = [...articles, ...sneakers, ...items].sort((a, b) => (b.ts || 0) - (a.ts || 0));
...
function renderHomeFeed(list) {
  ...
  wrap.innerHTML = list.map(p => {
    ...
    const onclickFn = p._type === 'article' ? `openArticle('${p.id}')`
                     ...
                     : `openItemModal('${p.id}')`;
    return `
    <div onclick="${onclickFn}" style="display:flex;gap:.7rem;background:var(--card);border:1px solid var(--bd);border-radius:10px;padding:.7rem;margin-bottom:.6rem;cursor:pointer;">
      ${img
